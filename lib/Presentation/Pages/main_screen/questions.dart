...
class _QuestionsState extends State<Questions> {
  ...
  Widget build(BuildContext context) {
    ...
    var avatar={'Primary':'lib/Presentation/Images/boysvg.svg',  
    ...
    List<Question> ques = [
      Question(ques:"Example sentence #1. I want to split it" , level:1 , isUrgent:true),
    ...
    return Scaffold(  
    appBar: scrBar(h),
    body:Container(height:h*0.7,
          child: scrBody(w, h, questionLevels, avatar, displayOptions, requestedList)), 
    );}
...
Stack scrBody(double w, double h, List<String> questionLevels, Map<String, String> avatar, List<String> displayOptions, ListView requestedList) {
    return Stack(
    clipBehavior: Clip.none,
    children:
    [Positioned(left: 0.25*w, top:-h*0.65 ,
      child:Opacity(  opacity:0.12,
        child:  Image.asset('lib/Presentation/Images/ArabicCircle.png', scale:1.5,)),
     ),
     Column(children:[ 
       levelMenu(w, questionLevels, avatar), 
       filterBar(w, displayOptions),
       Padding(padding:  EdgeInsets.symmetric(horizontal:w*0.05),  child: UnderLine(),),
       requestedList,
      ]), 
    ]);
  }

  Container levelMenu(double w, List<String> questionLevels, Map<String, String> avatar) {
    return Container(
         ...
         child: Padding(
           ...
           child: DropdownButton(
            value:questionLevel,
            underline: UnderLine(), 
            onChanged: (newValue){setState(() {questionLevel=newValue;});},
            items: questionLevels.map((String valueItem) {
              return DropdownMenuItem(
                value: valueItem,
                child: Row(children: [
                  SvgPicture.asset(avatar[valueItem], height:30, width :30,),
                  SizedBox(width: 8,),
                  Text(valueItem,  style:AppFonts.heading3.copyWith(color: AppColors.cDarkGrey[700],))],)
              );}).toList(),   
              ),
         ),
  ...
  return Container(
      child:Padding(
        ...
        child: Row(
          ...
          children:[
          Text("Sorted By",style:AppFonts.smallButtonText,),
          DropdownButton<String>(
          value:displayOption, 
          onChanged: (newValue){setState(() {displayOption=newValue;});},
          items: displayOptions.map((String valueItem) {
            return DropdownMenuItem<String>(
              value: valueItem,
              child: Row(children: [
                Text(valueItem,  style:AppFonts.smallButtonText.copyWith(fontSize: 15))],)
            );}).toList(),   
            ),
          Text("Urgent Only",style:AppFonts.smallButtonText,),
          Transform.scale(
            scale:0.75,
            child: Switch(
            value: showUrgent,
...
return ListView.builder(
  ...
  itemBuilder: (context,index){
    return Card(
      color: (index.isOdd)?null:Colors.blue[50],
      child :ListTile(
        title :Text(ques[index].ques , style: AppFonts.appText.copyWith(color:AppColors.cDarkGrey[900]),),
        trailing: Icon(
        ...
        leading: Icon(
              Icons.brightness_1, 
              color: (ques[index].isUrgent)?AppColors.cGreen[700]:AppColors.cDarkGrey[700],
              size: 24,),
        onTap: (){Navigator.pushNamed(context,'/Question',arguments:ques[index].ques);},
      ),
    );
   },
  );
